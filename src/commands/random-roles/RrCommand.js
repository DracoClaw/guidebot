...
module.exports = class RrCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!message.guild.me.hasPermission("MANAGE_ROLES"))
      return message.channel.send("I do not have 'MANAGE_ROLES' permission.");
    ...
    await message.channel
      ...
      .then((collected) => {
        try {
          ...
          roleMembers = message.guild.roles.cache
            .get(role)
            .members.map((m) => m);
        } catch (err) {
          flag = true;
          ...
          if (!collected)
            return message.channel.send("You ran out of time, Re-run command.");

          if (!message.guild.roles.cache.get(role))
            return message.channel.send("The mentioned role does not exist.");
    ...
    console.log(
      `users affected: ${roleMembers.length} | estimated time: ${estimation} mins`
    );
    ...
    message.channel.send(
      "Mention all roles you want in the randomization poll"
    );
    await message.channel
      .awaitMessages((m) => m.author.id == message.author.id, {
        max: 1,
      ...
      .then((collected) => {
        try {
          ...
          roles.shift();
          if (roles.length == 1)
            return message.channel.send("You must mention at least 2 roles.");
          ...
          for (let index = 0; index < roleMembers.length; index++) {
            if (
              !roleMembers[index].roles.cache.some((r) => roles.includes(r))
            ) {
              roleMembers[index].roles.add(
