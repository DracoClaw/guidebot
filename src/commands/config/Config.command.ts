    ...
    switch (subCommand) {
      case "get":
        ...
        this.getConfig(getParam, guild)
          .then((result) =>
            interaction.reply(`Parameter "${getParam}": ${result}`)
          )
          .catch((error) =>
            interaction.reply(`Failed to get "${getParam}" value: ${error}`)
          );
        break;
      case "set":
        ...
        this.setConfig(setParam, setValue, guild)
          ...
          .catch((error) =>
            interaction.reply(
              `Failed to update the "${setParam}" parameter due to the following reason: ${error}`
            )
          );
        break;
      default:
    ...
    if (update) {
      return new Promise((resolve) => {
        updateGuild(guild)
          .then((guild) => {
            console.log(
              `Guild "${guild.guildId}": Parameter ${parameter} updated with value "${value}"`
            );
            resolve(true);
          })
          .catch((error) => {
            console.error(
              `Failed to update Guild "${guild.guildId}": Parameter ${parameter} with value "${value}": ${error}`
            );
            resolve(false);
          });
      });
